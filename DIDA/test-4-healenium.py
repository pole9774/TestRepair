import time
from appium.options.android import UiAutomator2Options
from appium import webdriver
from appium.webdriver.common.appiumby import AppiumBy

desired_caps = {
    "platformName": "Android",
    "deviceName": "emulator-5554",
    "appPackage": "cn.ticktick.task",
    "appActivity": "com.ticktick.task.activity.MeTaskActivity",
    "noReset": True,
    "automationName": "UiAutomator2",
    "healenium:session": True,
	"sessionKey": "DIDA_test-4"
}

options = UiAutomator2Options()
for key, value in desired_caps.items():
    options.set_capability(key, value)

driver = webdriver.Remote("http://127.0.0.1:8085", options=options)

time.sleep(5)

# start from the Template (settings) page, enable template
# per v2: 'enable template' è scomparso, la pagina è Manage Template
try:
    el1 = driver.find_element(AppiumBy.XPATH, "//*[@text='Daily record']") # Healenium: UIAUTOMATOR selector changed to XPATH
    el1.click()
    time.sleep(3)

    el2 = driver.find_element(AppiumBy.ACCESSIBILITY_ID, 'More options')
    el2.click()
    time.sleep(3)

    el3 = driver.find_element(AppiumBy.XPATH, "//*[@text='Re-name']") # Healenium: UIAUTOMATOR selector changed to XPATH
    el3.click()

finally:
    time.sleep(5)
    driver.quit()
